...
import { renderGridColumns } from '../../column-utils/RenderUtils';
      ...
      <vaadin-split-layout class="full-size">
        ...
        <vaadin-context-menu .renderer="${this.renderContextMenu}" open-on="click" .listenOn="${this.menuButton}">
          ...
          <vaadin-grid
            ...
            ?multi-sort="${this.personGridConfiguration!.multisort}"
          >
    ...
    /*
      private getContextMenu(root: HTMLElement) {
          render(
            html`<vaadin-vertical-layout>
              ${this.personGridConfiguration!.columns.map((configuration) => {
                return html`<vaadin-checkbox ?checked="${configuration.visible}" @click="${() => this.toggleVisibility(configuration.id)}">${configuration.path}</vaadin-checkbox>`;
              })}
              </vaadin-vertical-layout>
            `,
            root as HTMLElement,
        ...
        //getContextMenu(this.personGridConfiguration, root, this, this.updateGridColumnConfigurationAndRefresh.bind(this));
      }

      private toggleVisibility(columnId: Number) {
        const columns: GridColumnConfigurator[] = [];
        // add all hidden columns
        this.personGridConfiguration!.columns.forEach( (column) => {
          const visible = (column.id == columnId)?!column.visible:column.visible;
